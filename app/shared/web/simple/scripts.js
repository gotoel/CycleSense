...
function updateStats() {
    fetch("/stats").then(response => response.json()).then(stats => {
        ...
        if (joystickL != null && joystickL.length == 1) {
            ...
            joystickAxisX = ((stats.chuck.axis_x) * (100) / 255) - 50;
            joystickAxisY = ((stats.chuck.axis_y) * (100) / 255) - 50;
        ...
        button_c.className = stats.chuck.button_c ?
            (stats.bike.rpm >= 40 ? "toggled" : "toggled-disabled") : "";
        button_z.className = stats.chuck.button_z ?
            (stats.bike.rpm >= 40 ? "toggled" : "toggled-disabled") : "";
